...
export class DocumentService {
  ...
  uploadGateDocument(caseId: string, formData: FormData): Observable<GateDocumentOut> {
    return this.http
      .post<GateDocumentOut>(`${this.apiUrl}/${caseId}/documents`, formData)
      .pipe(catchError(this.handleError));
  }

  getGateDocuments(caseId: string): Observable<GateDocumentOut[]> {
    return this.http
      .get<GateDocumentOut[]>(`${this.apiUrl}/${caseId}/documents`)
      .pipe(catchError(this.handleError));
  }

  getDocumentIntegrity(caseId: string, docId: string): Observable<DocumentIntegrityResult> {
    return this.http
      .get<DocumentIntegrityResult>(`${this.apiUrl}/${caseId}/documents/${docId}/integrity`)
      .pipe(catchError(this.handleError));
  }

  deleteDocument(caseId: string, docId: string): Observable<void> {
    return this.http
      .delete<void>(`${this.apiUrl}/${caseId}/documents/${docId}`)
      .pipe(catchError(this.handleError));
  }

  downloadDocument(caseId: string, docId: string): Observable<Blob> {
    return this.http
      .get(`${this.apiUrl}/${caseId}/documents/${docId}/download`, { responseType: 'blob' })
      .pipe(catchError(this.handleError));
  }
}
