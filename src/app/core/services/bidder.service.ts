...
export class BidderService {
  ...
  searchBidders(query: string): Observable<BidderSearchOut[]> {
    ...
    return this.http
      .get<BidderSearchOut[]>(`${environment.apiUrl}/cases/bidders`, { params })
  ...
  createBidder(data: BidderCreateSchema): Observable<BidderOut> {
    return this.http.post<BidderOut>(this.apiUrl, data).pipe(catchError(this.handleError));
  }

  getBidders(): Observable<BidderOut[]> {
    return this.http.get<BidderOut[]>(this.apiUrl).pipe(catchError(this.handleError));
  }
}
