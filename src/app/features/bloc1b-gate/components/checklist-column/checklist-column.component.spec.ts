...
describe('ChecklistColumnComponent', () => {
  ...
  it('devrait calculer correctement la progression avec des documents valides', async () => {
    ...
    const mockDocs = [
      { doc_type: 'BILAN', status: DocStatus.PRESENT },
      { doc_type: 'CPC', status: DocStatus.PRESENT },
      { doc_type: 'TFT', status: DocStatus.PRESENT },
      { doc_type: 'ATTESTATION_FISCALE', status: DocStatus.PRESENT },
    ...
    // Component matches by doc_type across all years (no fiscal_year filter)
  ...
  it('devrait ignorer les documents en statut REJECTED dans la progression', async () => {
    const mockDocs = [
      { doc_type: 'BILAN', status: DocStatus.REJECTED },
    ] as unknown as GateDocumentOut[];
