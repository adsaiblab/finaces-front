    ...
    this.yearlyProgress = fiscalYears.map((year) => {
      ...
      const docsForYear = this.requiredTypes.map((rt) => {
        ...
        const isUploaded = documents.some(
          (d) => d.doc_type === rt.type && d.status !== 'REJECTED',
        );
      ...
      return {
        ...
        docs: docsForYear,
        progressPercent:
          yearRequired > 0 ? Math.round((yearUploadedRequired / yearRequired) * 100) : 0,
      };
    });

    this.totalProgressPercent =
      totalRequired > 0 ? Math.round((totalUploadedRequired / totalRequired) * 100) : 0;
  }
