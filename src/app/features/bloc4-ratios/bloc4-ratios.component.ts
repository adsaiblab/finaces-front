...
export class Bloc4RatiosComponent implements OnInit {
    ...
    private loadMockData(): void {
        const mockResponse = {
                ...
                capacity: {
                    cash_flow_capacity: { current: 0.4, status: 'YELLOW', unit: 'ratio', variation_pct: -2 } as any,
                } as any,
                z_score: {
    ...
    public launchScoringAndPrediction(): void {
        ...
        forkJoin([mockMccScoreCall, mockIaPredictCall]).subscribe({
            ...
            error: () => {
                this.snackBar.open('An error occurred while launching predictions.', 'Close', { duration: 5000 });
                this.scoringInProgress.set(false);
            }
        });
    }
