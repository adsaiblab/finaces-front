...
describe('DashboardComponent', () => {
  ...
  async function buildComponent(serviceOverrides?: Partial<typeof mockCaseService>) {
    ...
    // Attendre que les async pipes souscrivent et que tap()/catchError() mettent a jour les signals
  ...
  // --- Creation de base ---
  it('devrait creer le composant dashboard', () => {
    expect(component).toBeTruthy();
  });

  it("devrait initialiser tous les signals d'erreur a null", () => {
  ...
  it('devrait initialiser tous les compteurs retry a 0', () => {
  ...
  // --- Loading signals ---
  it('isKpiLoading() devrait etre false apres emission du stream stats', () => {
    // whenStable() a attendu que le tap() du async pipe resolve le signal
    expect(component.isKpiLoading()).toBe(false);
  });

  it('isCasesLoading() devrait etre false apres emission du stream cases', () => {
    expect(component.isCasesLoading()).toBe(false);
  });

  it('isTensionsLoading() devrait etre false apres emission du stream tensions', () => {
    expect(component.isTensionsLoading()).toBe(false);
  });

  it('isChartLoading() devrait etre false apres emission du stream chart', () => {
    expect(component.isChartLoading()).toBe(false);
  });

  it("isKpiLoading() reste true si le stream stats ne s'est pas encore resolu", async () => {
    TestBed.resetTestingModule();
    // Subject ne emet pas immediatement -- simule un stream en attente
  ...
  it('tensionsLoaded() devrait etre vide quand getActiveTensionCases retourne []', () => {
    expect(component.tensionsLoaded()).toEqual([]);
  });

  it("tensionsEmpty() devrait etre true quand tensions = [] et pas d'erreur", () => {
    expect(component.tensionsEmpty()).toBe(true);
  });

  it('hasTensionsData() devrait etre false quand tensions = []', () => {
    expect(component.hasTensionsData()).toBe(false);
  });

  it('tensionsLoaded() devrait contenir les donnees recues', async () => {
    TestBed.resetTestingModule();
  ...
  it('devrait positionner statsError="server" lorsque getDashboardStats echoue', async () => {
    TestBed.resetTestingModule();
    await buildComponent({
  ...
  it('devrait positionner casesError="server" lorsque getRecentCases echoue', async () => {
    TestBed.resetTestingModule();
    await buildComponent({
  ...
  it('devrait positionner tensionsError="server" lorsque getActiveTensionCases echoue', async () => {
    TestBed.resetTestingModule();
    await buildComponent({
  ...
  it('devrait positionner chartError="server" lorsque getConvergenceChart echoue', async () => {
    TestBed.resetTestingModule();
    await buildComponent({
  ...
  it('isTensionsLoading() devrait etre false meme quand getActiveTensionCases echoue', async () => {
    TestBed.resetTestingModule();
    await buildComponent({
      getActiveTensionCases: vi.fn().mockReturnValue(throwError(() => new Error('500'))),
    });
  ...
  // --- Handlers retry ---
  it('onRetryStats() devrait incrementer statsRetryCount et vider statsError', () => {
  ...
  it('onRetryCases() devrait incrementer casesRetryCount et vider casesError', () => {
  ...
  it('onRetryTensions() devrait incrementer tensionsRetryCount et vider tensionsError', () => {
  ...
  it('onRetryChart() devrait incrementer chartRetryCount et vider chartError', () => {
  ...
  it("devrait afficher l'empty-state tensions quand la liste est vide", () => {
    // isTensionsLoading() = false (whenStable resolu), tensionsLoaded() = [] -> tensionsEmpty() = true
    fixture.detectChanges();
    const emptyState = fixture.debugElement.query(By.css('[data-testid="dashboard-tensions-empty"]'));
    expect(emptyState).toBeTruthy();
  });

  it('devrait afficher la carte tensions quand des donnees sont presentes', async () => {
    TestBed.resetTestingModule();
    await buildComponent({ getActiveTensionCases: vi.fn().mockReturnValue(of(mockTensions)) });
    fixture.detectChanges();
