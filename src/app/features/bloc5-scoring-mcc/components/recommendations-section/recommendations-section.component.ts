...
export class RecommendationsSectionComponent {
  ...
  public getRecommendationColorClass(type: string): string {
    switch (type) {
      ...
      default:
        return 'text-[color:var(--color-content-secondary)]';
    }
  }
  ...
  public getRecommendationBgClass(type: string): string {
    switch (type) {
      case 'POSITIVE':
        return 'bg-[color:var(--color-success)] bg-opacity-10 border-[color:var(--color-success)]';
      case 'WARNING':
        return 'bg-[color:var(--color-warning)] bg-opacity-10 border-[color:var(--color-warning)]';
      case 'CRITICAL':
        return 'bg-[color:var(--color-error)] bg-opacity-10 border-[color:var(--color-error)]';
      default:
        return 'bg-[color:var(--color-surface-default)] border-[color:var(--color-border-default)]';
    }
  }
}
